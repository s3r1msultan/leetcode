...
pub fn maximum_invitations(favorite: Vec<i32>) -> i32 {
    ...
    for i in 0..n {
        let favorite_person = favorite[i] as usize;
        indegree[favorite_person] += 1;
    }
    ...
    while let Some(person) = queue.pop_front() {
        let favorite_person = favorite[person] as usize;
        indegree[favorite_person] -= 1;

        depth[favorite_person] = depth[favorite_person].max(depth[person] + 1);

        if indegree[favorite_person] == 0 {
            queue.push_back(favorite_person);
        }
    }
