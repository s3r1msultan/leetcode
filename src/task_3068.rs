...
pub fn maximum_value_sum(nums: Vec<i32>, k: i32, edges: Vec<Vec<i32>>) -> i64 {
	fn max_sum_of_nodes(index: usize, is_even: usize, nums: &[i32], k: i32, memo: &mut Vec<Vec<i64>>) -> i64 {
		if index == nums.len() {
		...
		let xor_done = (nums[index] as i64 ^ k as i64) + max_sum_of_nodes(index + 1, is_even ^ 1, nums, k, memo);

		memo[index][is_even] = no_xor_done.max(xor_done);
		memo[index][is_even]
	}
...
fn test_maximum_value_sum() {
	...
	let k = 3;
	let result = 6i64;
	assert_eq!(maximum_value_sum(nums, k, edges), result);
